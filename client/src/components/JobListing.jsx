...
const JobListing = () => {
  const { isSearched, searchFilter, setSearchFilter ,jobs} = useContext(AppContext);
   const [showFilter,setShowFilter] = useState(false); // toggles sidebar filters
   ...
   const[selectedLocations, setSelectedLocations] = useState([]);// location filters
   const[filteredJobs,setFilteredJobs] = useState(jobs);// jobs after filtering

   const handleCategoryChange = (category)=>{
    setSelectedCategories(
      prev => prev.includes(category) 
      ? prev.filter(c => c !== category) //then remove it if already selected
       : [...prev,category]   //otherwise add it if not selected
    )
    ...
    <div className="container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8">
      ...
      <div className="w-full lg:w-1/4 bg-gray px-4">
        {/* Search Filter Hero Component*/}
        {
          ...
          //      AND at least one of the search filters (title or location) is not empty.
          isSearched &&
            (searchFilter.title !== "" || searchFilter.location !== "") && (
              <>
                <h3 className="font-medium text-lg mb-4 ">Current Search</h3>
                <div className="mb-4 text-gray-600">
                  {/* Conditional rendering for the title filter.If searchFilter.title has a value, then render: */}
                  {searchFilter.title && (
                  ...
                  {/* Applies !prev (logical NOT).If prev was false → it becomes true.
                       If prev was true → it becomes false. */}
      ...
      <section className="w-full lg:w-3/4 text-gray-800 max-lg:px-4">
        ...
        {/* pagination */}
        {/* Only shows pagination if there are jobs. */}
        {filteredJobs.length > 0 && ( 
            <div className="flex items-center justify-center space-x-2 mt-10">
                ...
                {/* Suppose you show 6 jobs per page.
                If there are 15 jobs → Math.ceil(15/6) = 3 pages.Loops through that fake array so you can render page numbers.
                 index + 1 gives you page numbers starting at 1. */}
                {Array.from({length:Math.ceil(filteredJobs.length/6)}).map((_,index)=>(
                    <a href="#job_list"  key={index}>
